  ...
  <div id="zong">
	...
	<div id="main">
     ...
     pageCurrent=Integer.parseInt(pageNo);
		...
		String sql="select* FROM 关注信息  WHERE user LIKE '"+"%"+user1+"%"+"'"; 
    ...
    <table align="center" frame="border" rules="all" width="1050px" height="500px">
		<caption style="font-size:30px">用户关注信息查询结果</caption>
    ...
    <th>关注信息</th>
      <th>操作</th>
		...
		if(rowCurrent>(pageCurrent-1)*pageSize&&rowCurrent<=pageCurrent*pageSize){
	        ...
	        <form method="post" action="HTyhgzxxcx1.jsp">
	        第<%=pageCurrent%>页 共<%=pageCount%>页&nbsp;
	        <%if(pageCurrent>1) {%>
	        <a href="HTyhgzxxcx1.jsp?pageno=1">首页</a>
	        <a href="HTyhgzxxcx1.jsp?pageno=<%=pageCurrent-1%>">上一页</a>
	        <%} %>&nbsp;
	        <%if(pageCurrent<pageCount){ %>
	        <a href="HTyhgzxxcx1.jsp?pageno=<%=pageCurrent+1%>">下一页</a>
	        <a href="HTyhgzxxcx1.jsp?pageno=<%=pageCount%>">尾页</a>
