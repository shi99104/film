  ...
  <body>
   <div id="zong">
     ...
     pageCurrent=Integer.parseInt(pageNo);
		...
		String sql="select* FROM 投诉建议信息 order by number desc"; 
		...
		<caption style="font-size:30px">投诉建议信息</caption>
    ...
    <th>时间</th>
      <th>操作</th>
		...
		if(rowCurrent>(pageCurrent-1)*pageSize&&rowCurrent<=pageCurrent*pageSize){
	        ...
	        <form method="post" action="HTtsjyxx.jsp">
	        第<%=pageCurrent%>页 共<%=pageCount%>页&nbsp;
	        <%if(pageCurrent>1) {%>
	        <a href="HTtsjyxx.jsp?pageno=1">首页</a>
	        <a href="HTtsjyxx.jsp?pageno=<%=pageCurrent-1%>">上一页</a>
	        <%} %>&nbsp;
	        <%if(pageCurrent<pageCount){ %>
	        <a href="HTtsjyxx.jsp?pageno=<%=pageCurrent+1%>">下一页</a>
	        <a href="HTtsjyxx.jsp?pageno=<%=pageCount%>">尾页</a>
