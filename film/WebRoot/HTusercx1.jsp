 ...
 */
  ...
  <div id="zong">
     ...
     pageCurrent=Integer.parseInt(pageNo);
		...
		String sql="SELECT * FROM 用户信息  WHERE user LIKE '"+"%"+user1+"%"+"'"; 
		...
		<caption style="font-size:30px">用户查询结果</caption>
	   ...
	   <td><a href="HTUserscProcess.jsp?number=<%=number%>">删除</a></td></tr>
	        ...
	        <form method="post" action="HTusercx1.jsp">
	        第<%=pageCurrent%>页 共<%=pageCount%>页&nbsp;
	        <%if(pageCurrent>1) {%>
	        <a href="HTusercx1.jsp?pageno=1">首页</a>
	        <a href="HTusercx1.jsp?pageno=<%=pageCurrent-1%>">上一页</a>
	        <%} %>&nbsp;
	        <%if(pageCurrent<pageCount){ %>
	        <a href="HTusercx1.jsp?pageno=<%=pageCurrent+1%>">下一页</a>
	        <a href="HTusercx1.jsp?pageno=<%=pageCount%>">尾页</a>
