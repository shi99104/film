  ...
  <body>
   <div id="zong">
     ...
     pageCurrent=Integer.parseInt(pageNo);
		...
		String sql="select* FROM 公告信息 WHERE user LIKE '"+"%"+user1+"%"+"'"; 
		...
		<caption style="font-size:30px">公告信息查询结果</caption>
		<tr align="center" valign="middle"><td colspan="4"><form action="HTggcx1.jsp">
...
<tr>
    ...
    <th>操作</th>
		...
		if(rowCurrent>(pageCurrent-1)*pageSize&&rowCurrent<=pageCurrent*pageSize){
				...
				%>
				<tr align="center" valign="middle"><td><%=announcement%></td><td><%=user%></td><td><%=time%></td><td><a href="HTggDelete.jsp?number=<%=number%>">删除</a></td>
             <%
	        }
	        rowCurrent++;
	        }
	        ...
	        <form method="post" action="HTggxx.jsp">
	        第<%=pageCurrent%>页 共<%=pageCount%>页&nbsp;
	        <%if(pageCurrent>1) {%>
	        <a href="HTggxx.jsp?pageno=1">首页</a>
	        <a href="HTggxx.jsp?pageno=<%=pageCurrent-1%>">上一页</a>
	        <%} %>&nbsp;
	        <%if(pageCurrent<pageCount){ %>
	        <a href="HTggxx.jsp?pageno=<%=pageCurrent+1%>">下一页</a>
	        <a href="HTggxx.jsp?pageno=<%=pageCount%>">尾页</a>
