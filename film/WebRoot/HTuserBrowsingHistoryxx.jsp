  ...
  <body>
   <div id="zong">
     ...
     pageCurrent=Integer.parseInt(pageNo);
		...
		String sql="select* FROM 历史信息 order by number"; 
      ...
      <input type="submit" value="搜索" style="width:50px;">
	   ...
	   <td><a href="HTuserBrowsingHistorysc.jsp?number=<%=number%>">删除</a></td></tr>
	        ...
	        <form method="post" action="HTuserBrowsingHistoryxx.jsp">
	        第<%=pageCurrent%>页 共<%=pageCount%>页&nbsp;
	        <%if(pageCurrent>1) {%>
	        <a href="HTuserBrowsingHistoryxx.jsp?pageno=1">首页</a>
	        <a href="HTuserBrowsingHistoryxx.jsp?pageno=<%=pageCurrent-1%>">上一页</a>
	        <%} %>&nbsp;
	        <%if(pageCurrent<pageCount){ %>
	        <a href="HTuserBrowsingHistoryxx.jsp?pageno=<%=pageCurrent+1%>">下一页</a>
	        <a href="HTuserBrowsingHistoryxx.jsp?pageno=<%=pageCount%>">尾页</a>
