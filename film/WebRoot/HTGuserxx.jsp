  ...
  <body>
   <div id="zong">
     ...
     pageCurrent=Integer.parseInt(pageNo);
		...
		String sql="select* FROM 管理员信息 order by number"; 
		...
		<caption style="font-size:30px">管理员信息</caption>
		<tr align="center" valign="middle"><td colspan="6"><form action="HTgusercx1.jsp">
      <input type="text" name="user" style="width:500px;" placeholder="请输入管理员名">
      <input type="submit" value="搜索" style="width:50px;">
		...
		if(rowCurrent>(pageCurrent-1)*pageSize&&rowCurrent<=pageCurrent*pageSize){
				...
				%>
	   <tr align="center" valign="middle"><td><%=number%></td><td><%=user1%></td><td><%=password%></td><td><%=time%></td>
	   <td><a href="HTGuserRevise.jsp?number=<%=number%>&user=<%=user1%>&password=<%=password%>&time=<%=time%>">修改</a></td>
	   <td><a href="GuserscProcess.jsp?number=<%=number%>">删除</a></td></tr>
	        ...
	        <%
	         ...
	         %>
	         <tr align="center" valign="middle"><td colspan="6">
	         <p align="center" class="p">
	        <form method="post" action="HTGuserxx.jsp">
	        第<%=pageCurrent%>页 共<%=pageCount%>页&nbsp;
	        <%if(pageCurrent>1) {%>
	        <a href="HTGuserxx.jsp?pageno=1">首页</a>
	        <a href="HTGuserxx.jsp?pageno=<%=pageCurrent-1%>">上一页</a>
	        <%} %>&nbsp;
	        <%if(pageCurrent<pageCount){ %>
	        <a href="HTGuserxx.jsp?pageno=<%=pageCurrent+1%>">下一页</a>
	        <a href="HTGuserxx.jsp?pageno=<%=pageCount%>">尾页</a>
