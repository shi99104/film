   ...
   <div id="zong">
	...
	<div id="main">
     ...
     pageCurrent=Integer.parseInt(pageNo);
		...
		String sql="select* FROM 投诉建议信息 WHERE user LIKE '"+"%"+user1+"%"+"'"; 
		...
		<caption style="font-size:30px">投诉建议信息查询结果</caption>
		<tr align="center" valign="middle"><td colspan="5"><form action="HTtsjyxxcx1.jsp">
    ...
    <th>时间</th>
      <th>操作</th>
		...
		if(rowCurrent>(pageCurrent-1)*pageSize&&rowCurrent<=pageCurrent*pageSize){
				...
				%>
				<tr align="center" valign="middle"><td><%=number%></td><td><%=user%></td><td><%=suggestion%></td><td><%=time%></td><td><a href="HTtsjyDelete.jsp?number=<%=number%>">删除</a></td>
             <%
	        }
	        rowCurrent++;
	        }
	         %>
	         <tr align="center" valign="middle"><td colspan="5"><p align="center" class="p">
	        <form method="post" action="HTtsjyxxcx1.jsp">
	        第<%=pageCurrent%>页 共<%=pageCount%>页&nbsp;
	        <%if(pageCurrent>1) {%>
	        <a href="HTtsjyxxcx1.jsp?pageno=1">首页</a>
	        <a href="HTtsjyxxcx1.jsp?pageno=<%=pageCurrent-1%>">上一页</a>
	        <%} %>&nbsp;
	        <%if(pageCurrent<pageCount){ %>
	        <a href="HTtsjyxxcx1.jsp?pageno=<%=pageCurrent+1%>">下一页</a>
	        <a href="HTtsjyxxcx1.jsp?pageno=<%=pageCount%>">尾页</a>
