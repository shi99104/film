  ...
  <body>
   <div id="zong">
     ...
     pageCurrent=Integer.parseInt(pageNo);
		...
		String sql="SELECT * FROM 历史信息  WHERE user LIKE '"+"%"+user1+"%"+"'"; 
		...
		<caption style="font-size:30px">用户电影浏览信息</caption>
		<tr align="center" valign="middle"><td colspan="5">
		<form action="HTuserBrowsingHistorycx.jsp">
      ...
      <input type="submit" value="搜索" style="width:50px;">
		...
		if(rowCurrent>(pageCurrent-1)*pageSize&&rowCurrent<=pageCurrent*pageSize){
				...
				%>
	   <tr align="center" valign="middle"><td><%=number%></td><td><%=user%></td><td><%=FilmTitle%></td><td><%=time%></td>
	   <td><a href="HTuserBrowsingHistorysc.jsp?number=<%=number%>">删除</a></td></tr>
	        <%
	        }
	        rowCurrent++;
	        }
	        ...
	        <form method="post" action="HTuserBrowsingHistorycx.jsp">
	        第<%=pageCurrent%>页 共<%=pageCount%>页&nbsp;
	        <%if(pageCurrent>1) {%>
	        <a href="HTuserBrowsingHistorycx.jsp?pageno=1">首页</a>
	        <a href="HTuserBrowsingHistorycx.jsp?pageno=<%=pageCurrent-1%>">上一页</a>
	        <%} %>&nbsp;
	        <%if(pageCurrent<pageCount){ %>
	        <a href="HTuserBrowsingHistorycx.jsp?pageno=<%=pageCurrent+1%>">下一页</a>
	        <a href="HTuserBrowsingHistorycx.jsp?pageno=<%=pageCount%>">尾页</a>
