   ...
   <div id="zong">
	...
	<div id="main">
     ...
     pageCurrent=Integer.parseInt(pageNo);
		...
		String sql="select* FROM 用户信息 order by number desc"; 
		...
		pageCurrent=1;
		%>
		<table align="center" frame="border" rules="all" width="1000px" height="500px">
		<caption style="font-size:30px">用户信息</caption>
		<tr align="center" valign="middle"><td colspan="8">
		<form action="HTusercx1.jsp">
             <input type="text" name="user" style="width:500px;" placeholder="请输入用户名称">
		...
		if(rowCurrent>(pageCurrent-1)*pageSize&&rowCurrent<=pageCurrent*pageSize){
				...
				%>
	   <tr  align="center"><td><%=number%></td><td><%=nickname%></td><td><%=user%></td><td><%=cipher%></td>
	   <td><%=mob%></td><td><%=time%></td>
	   <td><a href="HTuserxgProcess.jsp?number=<%=number%>&nickname=<%=nickname%>&user=<%=user%>&cipher=<%=cipher%>&mob=<%=mob%>&time=<%=time%>">修改</a></td>
	   <td><a href="HTUserscProcess.jsp?number=<%=number%>">删除</a></td></tr>
	        <%}
	        rowCurrent++;
	        }
	         %>
	        <tr align="center" valign="middle"><td colspan="8"><p align="center" class="p">
	        <form method="post" action="HTuserxx.jsp">
	        第<%=pageCurrent%>页 共<%=pageCount%>页&nbsp;
	        <%if(pageCurrent>1) {%>
	        <a href="HTuserxx.jsp?pageno=1">首页</a>
	        <a href="HTuserxx.jsp?pageno=<%=pageCurrent-1%>">上一页</a>
	        <%} %>&nbsp;
	        <%if(pageCurrent<pageCount){ %>
	        <a href="HTuserxx.jsp?pageno=<%=pageCurrent+1%>">下一页</a>
	        <a href="HTuserxx.jsp?pageno=<%=pageCount%>">尾页</a>
