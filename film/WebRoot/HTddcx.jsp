   ...
   <div id="zong">
	...
	<div id="main">
     ...
     pageCurrent=Integer.parseInt(pageNo);
		...
		String sql="SELECT * FROM 订单信息  WHERE FilmTitle LIKE '"+"%"+FilmTitle+"%"+"'"; 
      ...
      <input type="submit" value="搜索" style="width:50px;">
		...
		if(rowCurrent>(pageCurrent-1)*pageSize&&rowCurrent<=pageCurrent*pageSize){
				...
				%>
	    <tr align="center" valign="middle"><td><%=number%></td><td><%=ordernumber%></td><td><%=user%></td><td><%=FilmTitle1%></td><td><%=cinema%></td><td>￥<%=monovalent%>元</td>
	   ...
	   <td><a href="HTOrdernumberDelete.jsp?ordernumber=<%=ordernumber%>">删除</a></td></tr>
	        ...
	        <form method="post" action="HTddcx.jsp">
	        第<%=pageCurrent%>页 共<%=pageCount%>页&nbsp;
	        <%if(pageCurrent>1) {%>
	        <a href="HTddcx.jsp?pageno1">首页</a>
	        <a href="HTddcx.jsp?pageno=<%=pageCurrent-1%>">上一页</a>
	        <%} %>&nbsp;
	        <%if(pageCurrent<pageCount){ %>
	        <a href="HTddcx.jsp?pageno=<%=pageCurrent+1%>">下一页</a>
	        <a href="HTddcx.jsp?pageno=<%=pageCount%>">尾页</a>
