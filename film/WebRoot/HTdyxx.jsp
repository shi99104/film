  ...
  <body>
   <div id="zong">
     ...
     pageCurrent=Integer.parseInt(pageNo);
		...
		<caption style="font-size:30px">电影信息</caption>
	   ...
	   <td><a href="HTdyxxRevise.jsp?number=<%=number%>&FilmTitle=<%=FilmTitle%>&bill=<%=bill%>&intrommend=<%=intrommend%>&director=<%=director%>&actor=<%=actor%>&region=<%=region%>&laguae=<%=laguae%>&type=<%=type%>&releasedate=<%=releasedate%>&movietime=<%=movietime%>&price=<%=price%>">修改</a></td>
