...
public class addDiscussinfo extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//获取前端输入的评论信息
				String discuss= request.getParameter("discuss");//评论信息
				HttpSession session = request.getSession();
				String nickname= (String) session.getAttribute("nc");//用户昵称
				...
				if(discuss!=null&&FilmTitle!=null){						
					...
					t.AddDiscussinfo(t);
						response.sendRedirect("FilmDetailed.jsp");
							...
							request.setAttribute("pl", "评论不可为空");
							response.sendRedirect("FilmDetailed.jsp");
						}
				}else{
							request.setAttribute("message", "登录用户才能影评！");
